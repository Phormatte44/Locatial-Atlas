...
import {
  DEFAULT_LAB_RASTER_LAYER_ID,
  LAB_RASTER_LAYERS
} from "../presets/rasterLayers";

interface RasterLayerToggleProps {
  engine: AtlasEngine;
}
...
export function RasterLayerToggle({ engine }: RasterLayerToggleProps) {
  const [enabled, setEnabled] = useState(false);
  const [selectedLayerId, setSelectedLayerId] = useState(DEFAULT_LAB_RASTER_LAYER_ID);
  const [loadState, setLoadState] = useState<LayerLoadState | undefined>();

  useEffect(() => {
    for (const layer of LAB_RASTER_LAYERS) {
      engine.registerRasterLayer(layer);
    }
  }, [engine]);

  useEffect(() => {
    engine.setRasterLayers(enabled ? [selectedLayerId] : []);
  }, [engine, enabled, selectedLayerId]);

  useEffect(() => {
    return engine.onLayerLoadChange(({ state }) => {
      if (state.layerId === selectedLayerId && state.family === "raster") {
        setLoadState(state);
      }
    });
  ...
  const selectedLayer = LAB_RASTER_LAYERS.find((layer) => layer.id === selectedLayerId);

  return (
    <div style={rowStyle}>
      <span style={labelStyle}>Raster imagery (Lab demo)</span>
      ...
      {showRetry ? (
        <button
          ...
          onClick={() => {
            engine.retryLayerLoad(selectedLayerId, "raster");
          }}
        >
          Retry tile source
        </button>
