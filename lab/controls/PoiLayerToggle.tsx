...
import {
  DEFAULT_LAB_POI_LAYER_ID,
  LAB_POI_LAYERS
} from "../presets/poiLayers";
...
export function PoiLayerToggle({ engine }: PoiLayerToggleProps) {
  const [enabled, setEnabled] = useState(false);
  const [selectedLayerId, setSelectedLayerId] = useState(DEFAULT_LAB_POI_LAYER_ID);
  const [loadState, setLoadState] = useState<LayerLoadState | undefined>();

  useEffect(() => {
    for (const layer of LAB_POI_LAYERS) {
      engine.registerPoiLayer(layer);
    }
  }, [engine]);

  useEffect(() => {
    engine.setPoiLayers(enabled ? [selectedLayerId] : []);
  ...
  const selectedLayer = LAB_POI_LAYERS.find((layer) => layer.id === selectedLayerId);
  const clusteringEnabled = selectedLayer?.cluster?.enabled !== false;

  return (
    <div style={rowStyle}>
      <span style={labelStyle}>POI layers (Lab demo)</span>
      ...
      {showRetry ? (
        <button
          ...
          onClick={() => {
            engine.retryLayerLoad(selectedLayerId, "poi");
          }}
        >
          Retry URL load
        </button>
